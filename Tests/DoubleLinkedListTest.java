...
import eg.edu.alexu.csd.datastructure.linkedList.Classes.DoubleLinkedList;
...
class DoubleLinkedListTest {
    ...
    void set() {
        ...
        DoubleLinkedList newArr = new DoubleLinkedList();
        ...
        GroovyTestCase.assertEquals(newArr.get(1),subList.get(1));
        GroovyTestCase.assertEquals(newArr.get(0),subList.get(0));

        GroovyTestCase.assertEquals(newArr.sublist(-1,1),null);
    ...
    void assertArrays( ArrayList<Integer> originalArr, DoubleLinkedList newArr){
        assertEquals(originalArr.size(),newArr.size());
        for(int i=0; i<originalArr.size(); i++){
            GroovyTestCase.assertEquals(originalArr.get(i),newArr.get(i));
        }
    }
}